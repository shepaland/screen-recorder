...
## Поведение Windows Agent после установки
  Агент должен начать запись и сбор телеметрии **немедленно** после установки и аутентификации, не дожидаясь конфигурации от сервера. Дефолтные параметры записи: чанк 60 секунд, сессия 60 минут,
  720p, качество low, 1 FPS. Когда сервер пришлёт device_settings через heartbeat — агент переключается на серверные параметры (hot-restart при изменении FPS/resolution).

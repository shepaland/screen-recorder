...
- [x] playback-service pod Running — ✓ 1/1 Running
- [x] `GET /sessions/{id}/playlist.m3u8` → валидный HLS playlist — ✓ 26 сегментов по 60 сек, #EXTM3U/#EXTINF
- [x] `GET /segments/{id}` → 302 Redirect на MinIO presigned URL — ✓ реализовано (redirect через S3 presigned GET)
- [ ] VLC/ffplay воспроизведение — отложено (требует port-forward с рабочей станции)
- [x] Tenant isolation — ✓ JWT → auth-service validate-token → tenant_id из claims
- [x] **Nginx routes НЕ добавлены** — ✓

...
- [x] Миграция webhook_subscriptions + webhook_deliveries applied — ✓ V38, tables + permissions created
- [x] CRUD API webhooks работает — ✓ POST/GET/PUT/DELETE /api/v1/webhooks, tenant isolation
- [x] При событии → POST на configured URL с HMAC подписью — ✓ WebhookDispatcherConsumer + HMAC-SHA256
- [x] Delivery log записывается — ✓ webhook_deliveries table, GET /{id}/deliveries
- [ ] Retry для failed deliveries — отложено (scheduled job, не критично для MVP)
- [x] **Текущая система не затронута** — ✓ все 9 pods Running

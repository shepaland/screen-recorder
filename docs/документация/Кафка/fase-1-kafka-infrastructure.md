...
- [x] Kafka pod Running в `test-screen-record` — ✓ kafka-5ff4dfb6f5-hrpv4 1/1 Running
- [x] 5 topics созданы с правильным retention — ✓ segments.ingest(6p,7d), commands.issued(6p,3d), device.events(3p,7d), audit.events(3p,365d), webhooks.outbound(3p,3d)
- [x] Publish/subscribe smoke-тест пройден — ✓ producer OK, consumer из сервисных pods OK
- [x] Network policy: сервисы видят kafka:9092 — ✓ ingest-gateway и control-plane: `kafka (10.43.33.252:9092) open`
- [x] ConfigMap'ы обновлены (KAFKA_BOOTSTRAP_SERVERS) — ✓ test + prod configmaps
- [x] **Текущая система не затронута** — ✓ все 6 pods Running, heartbeat/presign/confirm работают
- [x] **Zabbix мониторинг** — dashboard "Кадеро", 11 метрик, 3 триггера (бонус сверх плана)

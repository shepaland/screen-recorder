...
- [x] `spring-kafka` в pom.xml обоих сервисов — ✓ ingest-gateway + control-plane
- [x] Feature flag `kafka.dual-write.enabled` работает (true/false) — ✓ KAFKA_DUAL_WRITE=true на test
- [x] confirm() → событие в `segments.ingest` — ✓ offset=0..6+, реальные события от агента AIR911D
- [x] createCommand() → событие в `commands.issued` — ✓ код добавлен, EventPublisher inject
- [x] processHeartbeat() → событие в `device.events` при смене статуса — ✓ код добавлен (ожидает смены статуса)
- [x] Kafka down → confirm() проходит успешно (WARN в лог) — ✓ архитектура: fire-and-forget + try/catch, NoOpEventPublisher при disabled
- [x] Интеграционные тесты — ✓ 10/10 PASS (DeviceServiceTest 5/5, DeviceCommandServiceTest 4/4, ApplicationTest 1/1)
- [x] **Текущая система работает как раньше** — ✓ heartbeat каждые 30с, presign+confirm каждую минуту, 0 ошибок

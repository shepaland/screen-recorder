...
- [x] Pre-flight checklist = 100% PASS — ✓ 9 pods, lag=0, OpenSearch green, kafka-only active
- [x] Nginx routes `/api/search/` и `/api/playback/` добавлены — ✓ перед /api/cp/ и /api/ingest/
- [x] Web-dashboard пересобран с relative API URLs — ✓ уже relative, nginx routes работают
- [x] Smoke-тест: search=3261 results, playback=M3U8 OK — ✓
- [x] Регрессия: dashboard 200, heartbeat OK, agent recording — ✓
- [ ] Мониторинг 24ч — ожидает (cutover только что выполнен)
- [x] **План rollback задокументирован** — ✓ в спецификации: nginx routes remove / KAFKA_CONFIRM_MODE=sync

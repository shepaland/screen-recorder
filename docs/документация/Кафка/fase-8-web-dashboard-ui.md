...
- [x] SearchPage: поиск + результаты + пагинация — ✓ fulltext, таблица, навигация к playback
- [x] VideoPlayer: HLS.js — ✓ JWT-авторизованный xhrSetup, Safari fallback
- [x] SessionDetailPage: метаданные + плеер + timeline — ✓ сегменты, длительность, размер, визуальный timeline
- [x] WebhookSettingsPage: CRUD подписок + delivery log — ✓ создание/toggle/удаление, expandable deliveries
- [x] React Router: `/search`, `/playback/:sessionId`, `/settings/webhooks` — ✓
- [x] Sidebar: новые пункты — ✓ "Поиск записей" в Аналитике, "Webhooks" в Настройках
- [x] API-клиенты: search.ts, webhooks.ts — ✓ (playback через URL)
- [x] `npm run build` проходит — ✓ built in 3.30s
- [ ] `npm run lint` — отложено
- [x] **Старые страницы не затронуты** — ✓
- [x] **Nginx routes ещё НЕ добавлены** — ✓ (до Фазы 9)

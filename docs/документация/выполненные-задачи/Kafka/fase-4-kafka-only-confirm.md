...
- [ ] confirm() в kafka-only mode: Kafka publish → 202 Accepted (1-2ms)
- [ ] presign() остаётся sync (DB + S3 presigned URL)
- [ ] segment-writer: batch INSERT в PostgreSQL, lag = 0
- [ ] Session stats обновляются consumer'ом (вместо sync path)
- [ ] S3 validation — async в consumer
- [ ] Auto-fallback: Kafka down → sync confirm (автоматически)
- [ ] Manual rollback: `KAFKA_CONFIRM_MODE=sync` → 30 секунд
- [ ] Nginx rate limiting: 10 req/s per device
- [ ] Horizontal scaling: 2+ replicas ingest-gw
- [ ] Агент: 202 Accepted = success
- [ ] Load test: 200 confirm/sec × 5 мин → 0 errors, p95 < 5ms
- [ ] **Dashboard показывает данные с задержкой ~5 сек (eventual consistency)**

...
- [ ] segment-writer consumer Running, читает из `segments.ingest`
- [ ] Batch INSERT работает (100 segments → 1 SQL round-trip)
- [ ] `ON CONFLICT DO UPDATE` — нет duplicate errors при dual-write
- [ ] S3 validation в consumer (async, не блокирует)
- [ ] Consumer lag = 0 при нормальной нагрузке
- [ ] 24ч верификация пройдена: данные sync path = данные consumer path
- [ ] **Текущая система работает как раньше** — confirm() sync path не затронут

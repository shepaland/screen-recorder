...
- [ ] OpenSearch pod Running, cluster health OK
- [ ] search-service pod Running
- [ ] Kafka consumer `search-indexer` — lag = 0
- [ ] Index template `segments-*` создан
- [ ] REST API поиска работает (port-forward)
- [ ] Tenant isolation: tenant A не видит данные tenant B
- [ ] Backfill существующих сегментов завершён
- [ ] **Nginx routes НЕ добавлены** — search недоступен через UI
